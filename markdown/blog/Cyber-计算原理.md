...
此外，法国数学家笛卡尔（1596～1650）和费马（1601～1665）各自独立发明了解析几何——即采用计算的方法去研究几何问题。解析几何规避了几何证明对于技巧和创造性的极高要求，将几何证明转化为按部就班的机械的计算。这与逻辑推理的机械化可以说是殊途同归。很难说哥德尔在证明不完备性定理时所使用的编码方法，没有受到他们的启发。
...
将某个包含皮亚诺算术的、递归可公理化的理论，称为“系统”。如果系统是ω-一致的（因而也是简单一致的），则系统是不完全的。
