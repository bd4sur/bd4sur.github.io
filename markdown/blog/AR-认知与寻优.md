...
**2025-06-28** 成功在自制Scheme解释器上运行神经网络：

- 3层的感知机（全连接网络），在鸢尾花数据集上执行小批量随机梯度下降训练，解决多分类问题。
- 完全用Scheme实现必要的算子，如矩阵乘、softmax等，没有外部依赖。所有算子都是ad-hoc的，没有任何优化，也没有边界条件检查，能用而已。
- 网络结构是硬编码的，更没有自动求导一类的黑科技。不过Scheme其实很适合做这种事情。
- 后续将JS的电子鹦鹉推理引擎封装起来，提供自回归生成层次上的Scheme接口。
- 动力系统真的很神秘，用同一个数据集对网络训练若干个epoch，loss在某个平台上徘徊了很久，随后在某个时刻突然掉下去，来到新的平台。整个梯度下降过程没有随机性，只有权重初始化是随机的。神经网络是对初值敏感的系统，权重初始化大有学问。

测试过程中发现虚拟机的GC可能有问题，但是重新跑了好久还没有复现。可能是哈希碰撞的问题。为了实现词法作用域，语法分析阶段对每个变量都展开成全局唯一的符号，但代价是符号的长度特别长。为了缩短符号长度，选用DJB哈希算法，将其压缩成较短的哈希值。然而由于哈希选型和截断方式有问题，尤其是因为没有加盐的缘故，很有可能是发生了碰撞，导致同名变量指向的对象被GC，发生运行时错误。

![ ](./image/G4/animac-mlp.jpg)
