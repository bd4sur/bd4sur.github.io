...
到了瓶颈，现在有几大问题有待解决：一是背压，二是抗丢包和和纠错，三是播放器的性能问题。

背压问题似乎要从Tx端server入手，准确计算发射NALU的频率，以稳定的速率发射，使其与空口带宽相匹配。昨天做了一个小改进，把NALU包改成定长1kB的。这样做的好处很多，也是MPEG-TS的做法。

抗丢包是个系统问题，根本上要靠改进信源编码和系统结构（例如引入主动重传机制）来解决。权宜之计有一些，例如通过改善背压问题来解决，现在可以做的是统计下物理层CRC校验不通过的原因，以及各种背压情况下的丢包率。纠错的话，GR的RS模块似乎有字节对齐的问题，这个模块的用法还需要仔细研究一下。交织也必须安排上。

至于播放器的性能问题，根本还是视频解码的问题。要定位性能瓶颈所在，实在不行暂时用现成的JPEG替换。

**2023-07-05**

今天在协议栈和视频编码设计上踩了一个巨大的深坑，获益良多。联想到主流视频编码标准的实际设计，立刻理解了他们这样做的原因。

音视频编码是本质复杂度特别高的一个技术领域，其方法和方法论层面上的复杂性，绝非吃饱了撑的，而是所解决问题的固有复杂性所决定的。当我耍小聪明试图避开这些复杂问题，立刻就会为我施加本质复杂度的天罚，使我堕入65535层的阿鼻地狱，然后不得不翻开天书一般的专著和标准文本，仔细聆听前辈的教诲，然后把它转化成上岸的代码天梯，继续迎接下一个坑。

以上说了很多，但其实什么都没说。不过这里想要指出的是，有一些鼎鼎大名的在ICT和所谓人工智能领域卓有成就的巨佬，似乎都有音视频编码的工作经历。音视频编码，说是信息通信领域的皇冠上的明珠，我觉得不过分。

**2023-07-05**

今天的经验教训：

- NALU通过物理信道传输，一旦出错，就会丢弃整个NALU。如果协议栈和编解码设计不好，丢失一个NALU会影响到很大的范围，导致上层协议更大的单元传输失败。
- 目前图像编码是按帧编码，极端情况下，一旦关键参数出错，则整帧无法解码。这就理解了为什么从H.261开始的视频编码标准要划分到所谓的条带这么细小的范围，就是为了限制偶发错误扩散到更大的范围。H.264更是将关键参数分开单独传输，进一步限制传输错误的影响范围。
- 上周末拍脑袋在CMS层加入校验机制，这是大错特错的。因为一旦CMS层的任何位置出错，即便实际上不影响解码（例如Padding部分错了），也会丢弃整个CMS帧，这会导致一整个音频帧和视频分片的丢失。当然，如果视频编码设计得当，丢失一个视频分片顶多出现一些马赛克，不会导致整帧解码失败；但是现在的设计是，丢失一个分片，就几乎一定会导致整帧解码失败。
- 一定要想方设法提高NALU的独立性，限制NALU出错的影响范围。因为实践证明丢失NALU的概率并不低。那么，既然物理层一帧出错就会丢弃一帧，可否继续把NALU拆分成更细小的单元？（所以说MTU的确定也是个很困难的问题）同时，还要尽量兼顾音视频传输的实时性问题。
- 想法：NALU不要无脑切割了，尽量让每个NALU都有相对独立而完整的意义。例如，MP3一帧最长960B，这实际上已经小于以太网的MTU了，在现有实现下，完全可以做到在一个NALU里完整地塞进去一个MP3帧，同时携带诸如时间戳、复用和同步的信息。这样一个NALU的含义和功能都是完整的，它如果丢弃了，顶多丢一个MP3帧，不会影响到其他的通道、视频帧等等。
- 印象里H.264的NALU更小，一百多B吧，这就契合了上面减小MTU的想法。当然，这里显然有个trade-off：减小MTU可以限制物理层丢帧的影响范围，但是会降低MTU的上层语义完整性，进而增加其错误扩散的可能性。这十分考验系统整体设计的智慧。

**2023-07-04**

音视频这个技术，其重要性在哪里，除了广电具有强烈的战略性的“国之重器”属性之外，它也是整个文化产业的物质技术基础。所以国资委管理的央企有个叫华录的，一开始就是搞磁带机、光盘机之类的，属于国家队。现在在搞什么就不知道了。

**2023-07-04**

音视频和图形学一直走在计算复杂度的最前沿，只是近年来深度学习开始逐渐对算力提出了更高要求。

我小时候正好赶上超级解霸的年代，用超级解霸在电脑上看《蓝猫淘气三千问》一类的VCD。当时什么都不懂，只是简单建立起一个直觉：用电脑播放音视频是天经地义的。后来，确切地说是前几年了解视频编码算法之后才知道，并不是。

VCD在音视频编码层面采用MPEG-1标准，其音频编码就是MP3的低复杂度版本“MP2”，在 ISO/IEC 11172 中定义。当年VCD编解码领域有那么几家专业厂商，比如斯高柏C-Cube等等，利用硬件ASIC解码MPEG音视频。VCD机如此，电脑也如此。

后来中央处理器算力上去以后，软件实时运行MPEG解码算法成为可能，于是出现了超级解霸，电脑无需安装硬件解码板卡。我也没有经历过硬解卡的年代。

但历史是个圈儿，如今无论电脑还是手机，几乎都有H.264等主流音视频标准的硬件编解码器。软件编解码完全是可行的，硬解的理由在于省电和更快。
音视频编码标准也意识到要控制自身的复杂度。于是AAC相比MP3，一方面划分了更细致的编码场景（以支持多种不同的复杂度），另一方面简化了编解码算法。H.265（HEVC）、H.266（VVC）也有意识地降低解码器复杂度（但编码器更复杂了，为了压缩效率逼近理论极限）。

说到纠错，上周末一个重要改进就是在CMS层加入了最简单的校验和，用来校验CMS帧是否损坏，损坏的帧直接丢弃，不会发送给播放器前端，这样就完全避免了播放器前端因为读到错误的数据帧（比如长宽65535）而卡住的情况。当然，数据帧之所以会损坏，现阶段由于是闭路测试，不涉及信道波动引发的错误，而主要是各级FIFO之间速率不匹配，主要是Tx端硬件接口环形队列承受较大的正向背压而触发丢帧流控（猜想）导致的。这不是用FEC就能简单解决的问题。当然，以后还是要加入FEC的。就用R-S码吧。

再回到超级解霸，小时候用电脑看VCD，确实挺容易彻底卡住的…光驱这东西优先级似乎很高，偏偏又是一种很慢的机械机构，所以被淘汰也是大势所趋吧。

**2023-07-03**

TODO 现在音频卡顿的重要原因就在于没有实现视频帧的分片机制。本来为了实现音频的实时传输，必须至少每24毫秒就要传输一个音频帧，为了充分利用带宽，可以在传输这一音频帧的同时传输一部分视频或者其他通道/业务的数据。这就需要对较长的视频帧进行切片。

然而现在偷了个懒，没有对视频帧作分片，而是直接把整个图像数据帧与音频帧拼接在一起。一个音频帧960B，一个图像数据帧按照9600B计算，NAL层的MTU按照1kB计算，则一个同时含有音视频通道的CMS帧需要11个NALU去传输。而从时间同步的关系看，后面接着3个左右的纯音频帧，每个对应1个NALU。这也就是说，如果信道带宽约等于音视频码流平均带宽，100ms传输14个NALU，其中前11个花费78ms，实际上只传输了1个音频帧，并且在24ms时间内就被播放完。后面22ms传输了3个音频帧，花费72ms时间播放。这之间存在巨大的空档，或许就是导致音频断续的原因所在。

**2023-07-03**

今天的进展，说大不大，说小不小。通信效果上没什么改善，一方面确实受限于机器性能，另一方面也是算法太粗糙。但正是在解决效果问题上，做了不少改进，也摸到了一些规律。

后续最重要的还是提升性能，目前来看，瓶颈在于接收机。接收机图像解码有很大的风险。视频编码算法还是要改进，现在太低效了。MPEG-1能够在1.5Mbps的码率下提供VCD画质的视频，我现在码率类似，但画质远逊于VCD。

五层协议栈全部实现了，但并非所有字段都有用到。近期有时间的话，开始研究OFDM替换QPSK。

**2023-07-01**

关于“名正言顺”的问题。我现在做的这个事情（用“项目”这个词要谨慎，因为一件事只有采用项目化方式进行运作时，才能称为“项目”），属于音视频通信，也属于电视广播的技术范畴。前者归工信部门管，后者归广电部门管。

通信和广播两个口子，对于同一件事情的理解还是有所不同。例如，ITU和MPEG制定的视频编码标准，就明显体现出通信和广播的理念差异。这个之前也讲过了，同样是“多媒体通信”，MPEG关注“多媒体”，而ITU关注“通信”。前者是广电口的思维，后者是邮电口的思维。就现在来说，邮电口显然占上风，ITU的标准事实上也更具有主导地位。例如H.264就是极为经典的视频编码标准。

2019年，也就是疫情爆发前，集中精力开发了自己的MP3编码器。当时，并没有给它正式命名。2020年6月份，研究视频编码的时候，突发奇想，考虑用拉丁文的词汇，为音视频研究的事情命名。

音频编码有个术语叫透明，指的是人耳听不出失真。于是选用Aqua给MP3编码器命名。这个单词是水的意思，希望编码器像水一样透明。而视频编码器则命名为Vita，这个词的意思是生命、活力、存在。视频就是运动图像序列嘛，是会动的，视觉也是长了眼睛的生物获取信息的主要渠道，所以选用这个词。而Aqua的A对应Audio，Vita的V对应Video，还是很巧合的。

至于现在正在做的这个东西应该叫什么，还应该想想。
...
突然领悟到，对于最底层的无线信道来说，它们传递的是符号，每个符号承载比特，而承载多少比特是由物理层决定的，因此也可以认为，物理层只能看到比特。而从比特到字节，这实际上已经是更高一层的语义了，物理层自己是无法判断字节边界的，必须交给上层处理。像RS码这种输入是字节的块，它不能运行在物理层和数据链路层之间，因为物理层不知何为字节。

而困难在于，用于提取字节边界的某些特征序列，它本身就会损坏，特征序列损坏，连字节都恢复不出来，遑论RS解码呢？所以现在看到了Unpacked码流的异步FEC解码模块，今天可以研究下怎么用。另外，还要解决神秘的背压匹配问题。
