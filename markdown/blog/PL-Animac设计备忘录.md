...
2023-08-21：用Scheme写一个字符串的中缀表达式解析器，作为测试用例。这个可以算是语言实现自举路上的一小步。Lexer和Parser是最麻烦的，但实际上没什么意思。正则语言同常见的形式系统一样，想要玩儿明白极不容易（例如匹配7的倍数的正则表达式），在正则语言上下功夫属于是狭义的“内卷”。
没指望自己做的语言实现能解决什么严肃问题，一个连GC都没有的运行时环境，眼睁睁地看着内存被吞噬掉，有什么好用的呢。
另外字符串这个东西无论在理论上还是实践上都不简单。理论上，只字符串匹配算法就足够让人头疼了。什么AC自动机啊，布隆过滤器啊，听起来就不好惹。实践上，例如，字符串这种东西究竟要不要做成可变的，分歧很大；再比如，字符串的底层并不简单，顾名思义，字符串是字符的序列，不是简单的字节序列，从字符到字节，中间有巨大的鸿沟，君不见多少语言掉进字符编码的大坑里。
Unix/Linux把字符串上升到某种哲学的高度（或许是王垠自己的观点）。我也觉得，计算机科学，某种程度上，甚至可以称为字符串或者字符流科学。
...
刚才所讲的callasync，很有可能就是大佬们70年代最初实现第一个Scheme解释器所用到的“actor”。callasync处理一般化的调用，实际上就是处理CPS的过程调用。那个传入的PC参数，实际上就是continuation。
...
我的Scheme实现至今没有自动垃圾回收。能不能让V8替我GC呢？恐怕是不行的。V8无法理解我的heap里面究竟在做什么。甚至我自己也不能理解。

于是想到哥德尔不完全性定理的最后一步证明，如果不断给有漏洞的理论打补丁，总能在打了补丁的理论中找到新的漏洞。说明这种漏洞是结构性的，不可能被简单地解决。就像内存管理问题一样，内存管理问题就是与主要问题伴生的一个结构性的“漏洞”，这个漏洞的根源跟哥德尔不完全性定理的根源是类似的——这样的系统无法完全理解自身。

## 2025-07-07

诸君如果审阅我的代码，就会发现我的实现完全是过程式的和C语言式的，所有的列表操作都是原位的，因此不怎么需要GC。把GC关掉之后，TPS从0.8上升到1.2。因此我现在实现的Scheme，比起原教纸的Scheme，更像JavaScript，它有Scheme的骨、JavaScript的肉、LISP的皮。Scheme的列表是点对的语法糖，而我的实现里面，列表是第一等公民。提供了set_item!和get_item两个原语，因此可以写图灵机式的代码。

## 2025-07-05自我评价

高情商：对于自研Scheme解释器增加各种实用特性这件事，我是十分克制的。Animac认同极简主义哲学，遵循“小内涵，大外延”和特性正交的原则，无必要不增实体，志在打造小而美的语言基础设施。

低情商：菜鸟票友归零对于PL的认识相当浅薄，浮于表面，对工业语言不加分析地模仿抄袭，对语言规范和二手资料断章取义不求甚解。何况他还缺乏必要的工程经验和工程能力，做出来的东西的性能和可靠性，远远劣于主流实现，劣化程度远超合理范围。他不是不想做那些好用的功能，实在是没有能力罢了。
