...
**2026-09-19**：DeepSeek-V4.1-Flash(-EXL3-2.9bpw) 双机Spark部署成功。Mia的配方有些小问题，不过AI全都搞定了。这是个参数量760B的巨大模型，极限量化后，确实能跑能用。至于性能，DSH统计的是28tok/s，不知是否还有优化空间。要知道这是个700B量级的模型，还要啥自行车啊。问了个电子行业的问题，仅凭自己的知识储备是可以正确回答这个问题，还不错。不过思考过程绕了很久，并且如果我的问题里有“不是中国的环保使用年限标志”这样的表述，会明显带偏它的思考过程。

![ ](./image/G4/ds-ds.jpg)

![ ](./image/G4/dsv41-dsh.png)

**2026-09-18**：我现在是从模型到挽具，全链路自有可控可审计。涉及真正个人隐私的东西，我都是在单独的一台干净主机上，用dsh或omp调用自有大模型处理。如有必要，完全可以断掉任何外网通信，完全自持运行。我现在用的一键启动机柜的那个东西就是用（自己做的！）500行的harness做的，也不是不能用嘛，所以我请大家先用聊天框搓一个最小可用harness出来，再让它自己迭代自我改进，左脚踩右脚上天，人人都有自己的harness它才安全！不是有个什么“一次性软件”的理念吗，所以什么zcode什么workbuddy为什么要用别人的呢，自己vibe一个然后左脚踩右脚上天不就好了嘛。

**2026-09-18**：户外架台没意思，户外VideCoding才有意思。下次再去长白山应该把电子式神带上。不过全套设备大概需要旅行箱那么大，包括移动电源、两台主机、路由器、笔记本、线缆。或许真的可以搞一个旅行箱大小的便携算力主机。关于户外移动算力主机这件事，我大概知道有些大学生会把ITX小主机之类的塞进旅行箱携行。但是我的需求更进一步：我要的是连电源也塞进箱子里，在任何地方都能无依托地自主运行的计算设施。实际上可以称之为野战算力节点了。那么电源就成了问题的关键。现在市面上的户外电源车载电源，形状上没有能完美塞进旅行箱的，大概只能自己手搓。并且考虑到能量密度，似乎也只能用锂电池。如果退一步，只要求户外车载移动算力，放弃对于单兵携行性的要求，那么现在市售的户外电源、电瓶、逆变器之类的，是可以满足要求的。最后还是要回到一个问题：为什么要把AI算力搬到野外呢？大概只是因为这样很酷吧……说白了，我无论玩什么东西，内心深处都有一个流浪要素：打造完全自持的核潜艇。

**2026-09-16**：一键召唤电子式神基本完成。核心是NUC5小主机，通过串口连接到ESP32-S3，控制433无线模块遥控空调、采集温湿度数据；NUC5同时与PDU通信，并充当视频网关。

![ ](./image/G4/dgxspark_dcim.jpg)

**2026-09-06**：DeepSeek的两篇纸在我看来是非常美而有潜力的：一个是《人类的本质是复读机》—Engram；另一个是《人类的本质是照相机》—DeepSeek-OCR。

**2026-08-10**：本地部署的DSv4f成功完成了第一个大规模编码任务——自研 Aqua MP3编码器，从JavaScript重构为C语言实现，并新增解码器。使用的AIAC干员是Codex，与DS的配合不是很好。主要有两点：一是Codex的语境窗口只有258k且不可调，因此没有充分发挥DS的1M语境窗口的优势。二是Codex经常会发出不完整的请求，导致推理后端报错，又不知问题出在哪，极大降低效率。总的来说，DSv4f处理复杂编码任务的能力，应该是值得信任的。

**2026-08-07**：2024简单对话→2025思维链→2026智能体。我在各种设备上部署LLM，起初LLM还只是简单的吐字，所以还能手工制作对话前端。后来o1思维链出现之后，也无非是解析think标签而已。而2025以来，LLM逐渐增强原生agentic能力，对话前端也成了没那么简单的agent，而不是简单的文字渲染。所以我废弃了那个粗糙的自制前端。到了现在，各种干员、各种缰绳已经相当成熟，自制更加没有意义了。

![ ](./image/G4/dgxspark.jpg)

**2026-01-02**：我从llama.cpp出现的第一时间开始，就认定它是在LLM的部署效率和运行效率之间取得平衡的最佳形态。llama.cpp本身可能不会笑到最后，但笑到最后的，一定长得很像llama.cpp。在我的创造实践中，我的电子鹦鹉虽不直接调用llama.cpp，但思路是一致的：**充分理解内存墙，充分信任仅权重量化，充分使用最直接普适的工程方法，充分发挥通用计算(CPU)的能力**。

**2025-12-27**：谈谈电子鹦鹉的GUI设计和实现。

我知道GUI是个非常复杂的东西，UI的复杂度从根本上说是业务+现实世界的复杂度。物理学有个**全息原理**，讲的是一个空间区域内的全部物理信息，可以完全编码在它的边界上。UI就是这个边界。

所以我一直对GUI、包括广义的前端技术，保持充分的敬畏。但越是这样，就越想从根本上去探索它、理解它，而不是简单地使用它。毕竟珠峰就在那里，为什么不试试呢？

我从2016年开始建设个人博客，从一开始就决定了将内容和呈现分离的实现原则，因此立刻着手开发自制的Markdown解析器。基本的Markdown大概是上下文无关语言，但实际上复杂得多，而我的实现非常粗糙且脆弱，因此我只能用到Markdown的一个简单子集，不过这对我来说完全够用了。不以辞害意、不纠结形式，也是我做博客的一个精神。

至于博客的整体框架，起初是简单的静态页面，每个文章对应一个HTML。后来，在我了解到SPA和PWA这些概念之前，就产生了将所有功能塞进同一个HTML页面，用XHR和大量的JS去实现呈现和交互逻辑的想法。这实际上已经是在做自己的前端框架了。2019年，基本上完成了SPA的改造，当时开发的框架，沿用至今，能够很好地满足我（有限的）需求。我自己实现的框架，极其简单，完全基于jQuery，不依赖任何复杂的构建系统。其实现思路和所有的设计细节，完全在我的头脑里，代码力求写成自解释的。并且，尽量不用时髦新特性，以满足将个人博客打造成电子遗产甚至电子墓碑的目标。这就是我一直不用Vue/React之类的前端框架的原因。

2025年开发电子鹦鹉，在GUI上面临更大的困难。我给我自己下达的开发任务是：从零开始，从屏幕驱动开始，完全自主实现所有的GUI交互逻辑，不使用任何诸如lvgl一类的GUI框架。

起初我没有任何思路，好在有LLM，跟LLM学到了很多东西。后来，我开始着手实现GUI，从屏幕驱动开始，快速堆砌功能，快速实现最小可用产品。而代价就是逻辑极其混乱。后来，花费大量的精力进行重构。主要改进包括：①将所有的GUI框架代码拆分成HAL、GFX和UI（widgets）三个逻辑层次；②逐步实现事件循环机制，确立清晰的状态转换和时间回调的基本框架。经过重构之后，GUI的逻辑变得相对清晰，功能、组件、抽象层次之间基本解耦，这样就可以很安全、很快捷地继续增加新功能。

电子鹦鹉GUI是我第一个嵌入式GUI工作，我的目标并非快速出产品，而是通过这项工作，理解GUI实现的底层原理。经过这段时间的实践，我认识到，GUI框架所做的事情，实际上跟操作系统（内核）非常相似——它们都是沟通赛博世界和物理世界的关口，必须直面物理世界的异步、随机、混沌和复杂。这对于我作为一个碳基人的生活观，也有很重要的启示。至此，我隐约理解了微软Windows为什么坚持为GUI赋予操作系统的 first-class citizen 地位。

电子鹦鹉 Nano-Pod 是一个顶天立地、具体而微、完全自持的系统。我将充分理解它、持续经营它，行动走在思想前面，满足我从小就有的对于任何能显示文字的便携电子设备的好奇心。

**2025-12-16**：电子鹦鹉分布式推理/流水线并行验证成功。

🔸左边ESP32-P4是主机，右边ESP32-S3是从机。
🔹主机负责处理：自回归生成循环、词元编码、采样、final_rmsnorm层、token_embd/cls层的计算，以及GUI和整体控制逻辑。
🔹从机负责处理：1个Transformer块的计算。
🔹主从之间通过I2C通信，传输Trm块之间的中间激活值。考虑到张量长度超过I2C缓冲区，做了简单的分帧和封装。从机每次计算1个token的1层，并返回结果。因此现在通信开销极大，推理性能劣化十倍以上。
🔹后面做成一主多从的星型网络，将模型权重分散到多个从机，实现较大模型的分布式推理部署。在批量推理场景下，还可以充分利用流水线并行，提升整体吞吐能力（？？？）
🔹当然，这个东西跟传说中的P-D分离是完全不沾边的。因为不管怎么折腾架构，内存带宽才是最根本的决定因素

![ ](./image/G4/llm-esp32-pp2.jpg)

![ ](./image/G4/llm-esp32-pp.jpg)

**2025-12-15**：把Qwen3-0.6B塞进ESP32显然是不现实的。但如果把它切成几份，放在多个ESP32上分布式推理呢？所以我打算研究一下简单的流水线并行，在多个极低资源设备上分布式推理较大模型的问题。现在看来，按层切分，接力推理，是比较可行的。

**2025-12-13**：继睡眠排序这一重大基础理论突破之后，归零又实现了排序算法的数智化转型——隆重推出基于大语言模型的AI排序算法：`ai_sort()`！现已集成到自研Scheme解释器，欢迎体验~

**2025-12-13**：成功把完整带GUI的电子鹦鹉移植到ESP32-P4上。近期在各种设备上移植电子鹦鹉，积累了不少经验：①ARM和x86除以0的行为的定义竟然不同。②各类设备I2C总线缓冲区的大小不同。③栈空间很小，需要十分细致的管理。④ESP-IDF默认char类型竟然是无符号的，其他的类型定义也很奇怪，或者说也没什么怪的，毕竟单片机。⑤没有特别调用ps_malloc，似乎也能很聪明地把大块内存分配到PSRAM，并且如果调用ps_malloc好像也能成功分配到4MB以上的空间。有待确认。总之单片机开发是螺蛳壳里做道场，是相当艰苦的思维体操。在资源丰富的Linux上开发的粗放的工作风格，在单片机上行不通。

**2025-12-10**：电子鹦鹉的GUI拆分成ui、gfx和hal三层之后，就可以在任何有128*64个点的东西上运行了，比如终端。

**2025-12-08**：比端侧更极端：ESP32单片机推理大语言模型。阶段回顾如下。

12月8日，在ESP32-P4上，成功部署230k参数的电子鹦鹉，生成速度可以达到24t/s。但是我不懂ESP32，内存分配细节现在还不清楚。部署过程中，AI帮了大忙，也帮了小小的倒忙。

在LLM🔥之前就买了一片ESP32，绝赞吃灰多年，听说它很强，一直想玩，但是毫无动力。至于电子鹦鹉，半年前实现了在路由器这样的嵌入式设备上的部署，但是这些设备还是太强了，上面跑着完整的Linux操作系统，有1GB以上的DDR内存和1GHz以上的CPU，跟PC其实没有质的区别。
...
**2025-12-06**：对电子鹦鹉做了比较重要的重构，为后续支持Q4km等奇怪量化做工程准备。但是我伤心地发现，之前训练的精神分析黑话小模型的原始权重找不到了。好在那些价值1克黄金的模型还在。

**2025-12-05**：最近有意把电子鹦鹉移植到ESP32上。我对ESP32并不了解，简单查了一下，一头雾水，规格谱系太复杂。同时我也没有底层开发经验，顶多用Arduino写一些应用代码。我现在要做的，是把电子鹦鹉中与操作系统相关的东西全部剥离出去，让逻辑的归逻辑，工程的归工程。当然，MCU的资源，尤其是RAM资源，实在有限。在上面跑LLM是不可能实用的。因此这只是整活而已。
